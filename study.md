...
```md
It does print. No need to fix anything.  The output is "The star is approximately 40170261586578.086 kilometers away".```
...
```md
It does not print. We need to change mass_kg to @mass_kg and then make the same change in the print variable interpolation. Now it says "The star has a mass of approximately 2.4477e+29 kilograms."
...
```md
No, modifying sol will not modify the_sun.  They are both instance variables of the same Star class, but are not directly connect to each other.  Changes to the Star method will effect both of them, but changing one will have no effect on the other. ```
...
```md
Yes, modifying the_sun will affect the nearest star.  nearest_star is pointing to the same memory location and has the same memory id number.
```
...
```javascript
const vega = {distance: 25.04, solarMass: 2.135}
```
