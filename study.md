...
```md
Yes it should, it inherits the properties of the Star object.
```
...
```md
Nope! But if you add @ signs in the def and in the initialization that should fix it.
You're making the variable an instance variable in that case.
```
...
```md
No, both of these objects are created as separate object.
```
...
```md
No, setting nearest_star to the_sun points nearest_star to the data that was created
with the_sun but when you redfine the sun it just creates new data and leaves nearest_star
pointing at the original data. (I think) update: solution branch says I'm wrong, can we
talke about this?
```
...
```md
You'd be creating a hash using hash literal notation. 
```
