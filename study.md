...
I believe it works becuase it was properly declared using @ (as a class varible)
```
...
```md
It does not. It can be fixed, however, by properly delcaring it with @, then changing the interpolation accordingly.
```
...
```md
No, because it is a seperate entity. They are both part of the the Star class,
and inheret the properties associated with that class but are seperate instances of 'Stars'
```
...
```md
Yes, because the sun IS the nearest_star as well. The nearest_star inherets its properties from the_sun and any changes will apply to both 'Star' instances.
```
