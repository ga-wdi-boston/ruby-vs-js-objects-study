...
```md
yes, it does run.  the scope of @distance_km is the instantiated object
```
...
```md
no, it does not run.  mass_kg is local to the initialize method.  to fix it,
make it an instance variable by putting an @ in front of it (in initialize and in the interpolated string)
```
...
```md
no - they are two different objects
```
