...
```md
no. returns name error. undefined local variable
need to properly define mass_kg
@mass_kg = mass_solar * (1.99 * 10 ** 30) and
 #{@mass_kg} then will print properly
```
...
```md
no because they are defined separately
```
...
```md
yes because the_sun is defined with the nearest_star, they have the same value
```
