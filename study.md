...
```md
Yes, distance_km will work to output "The star is approximately (4e16) kilometers away"
```
...
```md
No, it needs an "@" when it is initialized.
```
...
```md
No, while they have the same value they are different variables.
```
...
```md
Yes, because ruby uses pass by reference, meaning the variables point to the
memory location and not a new copy of the (object) being referenced.
```
