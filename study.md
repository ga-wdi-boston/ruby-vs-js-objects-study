...
```md
Yes.  it gets access to the variable @distance_km.
...
```md
Nope. mass_kg isn't a variable that is available globally.
You can  set @mass_kg in the constructor.
```
...
```md
They're are 2 different objects even though they have the same attributes.
```
...
```md
Yes because `nearest_star` is referencing `the_sun`
```
