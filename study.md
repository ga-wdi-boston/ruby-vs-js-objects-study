...
```md
It will print because print_distance is defined inside the class.  The result will be the product of 4.246 and 9_460_730_472_580.8
```
...
```md
This will not print anything because mass_kg is not an instance variable, it is a variable defined in the local scope of initialize.  If it is created with an @ sign to make it an instance variable then it will work.
```
...
```md
Modifying 'sol' will not affect 'the_sun' because they are separate instances of Star.
```
...
```md
Modifying `the_sun` will affect `nearest_start` because they are both references to the same object.
```
