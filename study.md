...
```md
Yes, it runs because methods created within an object are public by default.
```
...
```md
It does not work.  You need to add an @ before mass_kg on lines 25 and 33.
```
...
```md
I don't believe so.  The only connection is that they were created by the same class,
 but once they are created they are separate.
```
...
```md
Yes, they reference the same object.
```
