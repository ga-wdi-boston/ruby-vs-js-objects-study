...
```md
This will run, however, the value returned is nill.
...
```md
sol and the_sun are independent of each other. they have different object_ids
```
...
```md
the_sun and the nearest_star share the same object_id
any changes to one will effect the other.
```
...
```md
with the syntax, potentially a hash would be cretaed with two symbols
```
